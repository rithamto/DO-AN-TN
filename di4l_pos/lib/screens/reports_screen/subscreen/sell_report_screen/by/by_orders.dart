...
class ByOrders extends StatefulWidget {
  const ByOrders({Key? key}) : super(key: key);
  static BlocProvider<SellReportCubit> provider() {
    return BlocProvider(
      create: (context) => SellReportCubit(),
      child: const ByOrders(),
    );
  }

  @override
  State<ByOrders> createState() => _ByOrdersState();
}

class _ByOrdersState extends State<ByOrders> with AfterLayoutMixin {
  ...
  FutureOr<void> afterFirstLayout(BuildContext context) {
    context.read<SellReportCubit>().getOrders();
  }
    ...
    return BlocBuilder<SellReportCubit, SellReportState>(
      builder: (context, state) {
        ...
        for (int i = 0; i < state.data!.orders.length; i++) {
          totalFinalTotal += [state.data!.orders[i].finalTotal];
        }
  ...
  Widget buildHistory() {
    return BlocBuilder<SellReportCubit, SellReportState>(
        builder: (context, state) {
      final orderReports = state.data!.orders;
      return orderReports.isNotEmpty
          ? Expanded(
              child: Container(
                ...
                child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      if (index < orderReports.length) {
                        return ByOrderWidget.provider(
                          orders: orderReports.elementAt(index),
                        );
                      } else {
